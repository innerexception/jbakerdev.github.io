...
/***/ function(module, exports, __webpack_require__) {
	...
	var _AppReducer = __webpack_require__(185);
	...
	var store = (0, _redux.createStore)(_AppReducer2.default, (0, _redux.applyMiddleware)(_reduxThunk2.default // lets us dispatch() functions
	));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _browserFilesaver = __webpack_require__(186);
	...
	//
	//import { updateViewStateDeleteActiveShape, updateViewStateDeleteOverShape, updateViewStateOverShape,
	//         updateViewStateActiveLensCategoryOrdinal, updateViewStateActiveLensCategorySetting, updateViewStateActiveLensSticky,
	//         updateViewStateShowFilterOptionsForShape, updateViewStateDeleteConfigWindow, getInitialViewState, updateViewStateLensIn } from './ViewStateHelper.js';
	//
	//import { updateMarkersGlobalCategoryChanged, updateMarkerStateIn, updateMarkersShapeChanged, updateMarkersShowMetaInfoPopup,
	//         updateMarkersZoomLevelChanged, updateAllMarkersShapes, insertLonLat, updateShapesRemoveShape, updateShapesShapeAdded,
	//         updateMarkersSliceCategoryChanged } from '../marker/MarkerHelper.js'
	...
	var appReducer = function appReducer() {
	    ...
	    switch (action.type) {
	        case 'FETCH_VIEW_STATE':
	            return _extends({}, state, { viewState: getInitialViewState(action.viewState), markers: [] });
	        case 'MARKER_CLICK':
	            return _extends({}, state, { markers: updateMarkersShowMetaInfoPopup(state.markers, action.marker) });
	        case 'SHAPE_CREATED':
	            panMapToBounds(action.map, action.shape);
	            var categoryName = getFirstValidCategoryName(state.categories, state.dataCategories);
	            var firstValue = getFirstCategoryValue(state.categories, categoryName);
	            if (!action.shape.lensOptions) {
	                action.shape.lensOptions = {
	                    selectedCategories: new Map(),
	                    color: Math.floor(Math.random() * 254 + 1) + ',' + Math.floor(Math.random() * 254 + 1) + ',' + Math.floor(Math.random() * 254 + 1),
	                    sticky: false
	                };
	            }
	            action.shape.lensOptions.selectedCategories.set(categoryName, firstValue);
	            var category = state.categories.filter(function (category) {
	                return category.name === categoryName;
	            })[0];
	            if (category.min) action.shape.lensOptions.ordinal = '<';
	            state.viewState.activeShapeConfigPanel = action.shape;
	            return _extends({}, state, { shapes: updateShapesShapeAdded(state.shapes ? state.shapes : [], action.shape), markers: updateMarkersShapeChanged(state.markers, action.shape), viewState: updateViewStateActiveLensCategorySetting(state.viewState, categoryName, firstValue) });
	        case 'SHAPE_RIGHT_CLICKED':
	            return _extends({}, state, { viewState: updateViewStateShowFilterOptionsForShape(state.viewState, action.shape) });
	        case 'SHAPE_BOUNDS_CHANGED':
	            return _extends({}, state, { markers: updateMarkersShapeChanged(state.markers, action.shape) });
	        case 'RADIUS_CHANGED':
	            return _extends({}, state, { markers: updateMarkersShapeChanged(state.markers, action.shape) });
	        case 'ZOOM_LEVEL_CHANGED':
	            return _extends({}, state, { markers: updateMarkersZoomLevelChanged(state.markers, action.e) });
	        case 'SHAPE_DELETE':
	            return _extends({}, state, { shapes: updateShapesRemoveShape(state.shapes, action.shape), viewState: updateViewStateDeleteConfigWindow(state.viewState, action.shape), markers: updateAllMarkersShapes(state.shapes, state.markers, action.shape, state.categories) });
	        case 'DATA_FETCHED':
	            return _extends({}, state, { markers: action.markers });
	        case 'FETCHED_DATA_NO_GEOCODE':
	            var name = getFirstValidCategoryName(action.categories, action.dataCategories);
	            return _extends({}, state, { categories: updateCategoriesGlobalCategory(action.categories, name, 'both'), markers: insertLonLat(action.rows, action.categories, name), dataCategories: action.dataCategories });
	        case 'MARKER_IN':
	            return _extends({}, state, { markers: updateMarkerStateIn(state.markers, action.marker) });
	        case 'LENS_IN':
	            return _extends({}, state, { viewState: updateViewStateLensIn(state.viewState) });
	        case 'SET_LENS_CATEGORY_SETTING':
	            var cat = state.categories.filter(function (category) {
	                return category.name === action.category;
	            })[0];
	            var ordinal = typeof cat.min !== 'undefined' ? '<' : null;
	            return _extends({}, state, { viewState: updateViewStateActiveLensCategorySetting(state.viewState, action.category, action.setting), markers: updateMarkersShapeChanged(state.markers, state.viewState.activeShapeConfigPanel, null, ordinal) });
	        case 'SHAPE_STICKY_CHANGED':
	            return _extends({}, state, { viewState: updateViewStateActiveLensSticky(state.viewState, action.state) });
	        case 'GLOBAL_CATEGORY_CHANGE':
	            return _extends({}, state, { categories: updateCategoriesGlobalCategory(state.categories, action.category, action.categoryType), markers: updateMarkersGlobalCategoryChanged(state.markers, action.category, state.categories, state.shapes) });
	        case 'SHOW_SHAPE_CONFIG_BUTTON':
	            return _extends({}, state, { viewState: updateViewStateOverShape(state.viewState, action.shape) });
	        case 'HIDE_SHAPE_CONFIG_BUTTON':
	            return _extends({}, state, { viewState: updateViewStateDeleteOverShape(state.viewState) });
	        case 'HIDE_SHAPE_CONFIG':
	            return _extends({}, state, { viewState: updateViewStateDeleteActiveShape(state.viewState) });
	        case 'SET_LENS_ORDINAL':
	            return _extends({}, state, { viewState: updateViewStateActiveLensCategoryOrdinal(state.viewState, action.category, action.setting), markers: updateMarkersShapeChanged(state.markers, state.viewState.activeShapeConfigPanel) });
	        case 'EXPORT_CSV':
	            exportCsv(state.markers, state.categories);
	            return _extends({}, state);
	        case 'SLICE_CATEGORY_CHANGED':
	            return _extends({}, state, { viewState: updateViewStateSliceCategoryChanged(state.viewState, action.categoryName, action.selectChildren), markers: updateMarkersSliceCategoryChanged(state.markers, action.categoryName, state.shapes, state.viewState.activeSliceCategories, action.selectChildren) });
	        case 'TOGGLE_PARENT_CATEGORY':
	            return _extends({}, state, { viewState: updateViewStateOpenParentRegions(state.viewState, action.categoryName) });
	        default:
	            return state;
	    }
	};
	
	var updateViewStateOpenParentRegions = function updateViewStateOpenParentRegions(viewState, parentName) {
	    var newState = _extends({}, viewState);
	    if (!newState.openParentRegions) newState.openParentRegions = {};
	    newState.openParentRegions[parentName] = !newState.openParentRegions[parentName];
	    return newState;
	};
	
	var updateViewStateSliceCategoryChanged = function updateViewStateSliceCategoryChanged(viewState, categoryName, selectChildren) {
	    var newState = _extends({}, viewState);
	    if (!newState.activeSliceCategories) newState.activeSliceCategories = {};
	    newState.activeSliceCategories[categoryName] = !newState.activeSliceCategories[categoryName];
	
	    if (selectChildren) {
	        PACountyRegionsMap[categoryName].forEach(function (child) {
	            newState.activeSliceCategories[child] = true;
	        });
	    }
	...
	var exportCsv = function exportCsv(markers, categories) {
	    var activeMarkers = markers.filter(function (marker) {
	        return marker.isActive;
	    });
	    var csvText = activeMarkers.map(function (marker) {
	        return categories.map(function (category) {
	            return marker[category.name].split(',').join('').split('\n').join('');
	        }).join(',');
	    }).join('\n');
	
	    var headers = categories.map(function (category) {
	        return category.name;
	    }).join(',');
	
	    var blob = new Blob([headers + '\n' + csvText], { type: 'text/plain;charset=utf-8' });
	    _browserFilesaver2.default.saveAs(blob, 'DataSquidExport_' + new Date().toLocaleString() + '.csv');
	};
	
	var getFirstCategoryValue = function getFirstCategoryValue(categories, categoryName) {
	    return categories.filter(function (category) {
	        return category.name === categoryName;
	    })[0].values[0];
	};
	
	var getFirstValidCategoryName = function getFirstValidCategoryName(categories, dataCategories) {
	    var validCategories = Array.from(categories);
	    var i = -1;
	    validCategories = validCategories.filter(function (category) {
	        i++;
	        return dataCategories[i] === 'CATEGORY' || dataCategories[i] === 'LENS';
	    });
	    return validCategories[0].name;
	};
	
	var updateCategoriesGlobalCategory = function updateCategoriesGlobalCategory(categories, categoryName, categoryType) {
	    var newCategories = Array.from(categories);
	
	    newCategories.forEach(function (category) {
	        if (category.name === categoryName) {
	            if (categoryType === 'both') {
	                category.activeSize = true;
	                category.activeColor = true;
	            } else {
	                category[categoryType] = true;
	            }
	        } else delete category[categoryType];
	    });
	
	    return newCategories;
	};
	
	var panMapToBounds = function panMapToBounds(map, shape) {
	    map.panTo(shape.getBounds().getCenter());
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _UIStateContainer = __webpack_require__(190);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var mapStateToProps = function mapStateToProps(state) {
	    return {
	        ...
	        markers: state.markers ? state.markers : [],
